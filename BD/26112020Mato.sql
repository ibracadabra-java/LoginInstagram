--------------------------------------------------------
-- Archivo creado  - jueves-noviembre-26-2020   
--------------------------------------------------------
...
--------------------------------------------------------
   ...
   CREATE SEQUENCE  "APIINST"."SE_ID_ANDROIDVERSION"  MINVALUE 1 MAXVALUE 9999999999999999999999999999 INCREMENT BY 1 START WITH 14 NOCACHE  NOORDER  NOCYCLE ;
--------------------------------------------------------
...
--------------------------------------------------------
   ...
   CREATE SEQUENCE  "APIINST"."SE_ID_PROXY"  MINVALUE 1 MAXVALUE 9999999999999999999999999999 INCREMENT BY 1 START WITH 14 NOCACHE  NOORDER  NOCYCLE ;
--------------------------------------------------------
...
OPEN X_CURSOR FOR 
SELECT
ML.USUARIO, ML.PASS, ML.ID_PROXY, ML.DEVICE,ML.PK,
DEV.ADID, DEV.ANDROIDBOARDNAME, DEV.ANDROIDBOOTLOADER, DEV.ANDROIDVER,DEV.DEVICEBRAND,
DEV.DEVICEGUID, DEV.DEVICEMODEL, DEV.DEVICEMODELBOOT, DEV.DEVICEMODELIDENTIFIER,
DEV.DPI, DEV.FIRMWAREFINGERPRINT, DEV.FIRMWARETAGS, DEV.FIRMWARETYPE, DEV.RESOLUTION,
ANDV.APILEVEL, ANDV.CODENAME, ANDV.VERSIONNUMBER,
PR.ADDRESSPROXY, PR.PASSPROXY, PR.USERPROXY
FROM MLOGIN ML
INNER JOIN DEVICE DEV ON DEV.DEVICEID = ML.DEVICE
INNER JOIN ANDROID_VERSION ANDV ON ANDV.IDANDROIDVERSION = DEV.ANDROIDVER
INNER JOIN PROXY PR ON PR.ID_PROXY = ML.ID_PROXY
where ML.PK = X_PK;
...
SELECT COUNT(PK) INTO X_EXISTE FROM MLOGIN WHERE PK = X_PK;
 IF X_EXISTE = 0 THEN
...
SET
CODENAME = X_CODENAME,
VERSIONNUMBER = X_VERSIONUMBER,
APILEVEL = X_APILEVEL
WHERE ANDROID_VERSION.IDANDROIDVERSION = X_IDANDROIDVERSION;

UPDATE DEVICE
SET 
ADID = X_ADID,
ANDROIDBOARDNAME = X_ANDBOARDNAME,
ANDROIDBOOTLOADER = X_ANDBOOTLOADER,
ANDROIDVER = X_IDANDROIDVERSION,
DEVICEBRAND = X_DEVICEBRAND,
DEVICEGUID = X_DEVICEGUID,

DEVICEMODEL = X_DEVICEMODEL,
DEVICEMODELBOOT = X_DEVICEMODELBOOT,
DEVICEMODELIDENTIFIER = X_DEVICEMODELIDENTIFIER,
DPI = X_DPI,
RESOLUTION = X_RESOLUTION,
FIRMWAREFINGERPRINT = X_FIRMWAREFINGERPRINT,
FIRMWARETAGS = X_FIRMWARETAGS,
FIRMWARETYPE = X_FIRMWARETYPE
WHERE DEVICEID = X_DEVICEID;

UPDATE PROXY
SET
ADDRESSPROXY = X_ADDRESSPROXY,
USERPROXY = X_USERPROXY,
PASSPROXY = X_PASSPROXY
WHERE ID_PROXY = X_ID_PROXY;
...
BEGIN
BEGIN
SELECT ID_PROXY INTO X_PROXY FROM APIINST.MLOGIN WHERE APIINST.MLOGIN.PK = X_PK;
SELECT DV.ANDROIDVER INTO X_ANDROIDVER FROM DEVICE DV
INNER JOIN MLOGIN ML ON ML.DEVICE = DV.DEVICEID
WHERE ML.PK = X_PK;
DELETE FROM ANDROID_VERSION
WHERE IDANDROIDVERSION = X_ANDROIDVER;

DELETE FROM APIINST.PROXY
WHERE APIINST.PROXY.ID_PROXY = X_PROXY;

--SELECT DV.DEVICEID INTO X_DEVICE FROM DEVICE DV
--INNER JOIN MLOGIN ML ON ML.DEVICE = DV.DEVICEID
--WHERE ML.PK = X_PK;
--DELETE FROM DEVICE
--WHERE DEVICEID = X_DEVICE;

--DELETE FROM APIINST.MLOGIN
--WHERE APIINST.MLOGIN.PK = X_PK;
...
--------------------------------------------------------
  ...
  ALTER TABLE "APIINST"."MLOGIN" MODIFY ("ID_PROXY" NOT NULL ENABLE);
...
--------------------------------------------------------
  ...
  ALTER TABLE "APIINST"."MLOGIN" ADD CONSTRAINT "PROXY_FK" FOREIGN KEY ("ID_PROXY")
	  REFERENCES "APIINST"."PROXY" ("ID_PROXY") ON DELETE CASCADE ENABLE;
--------------------------------------------------------
